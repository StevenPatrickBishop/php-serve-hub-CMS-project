...
if ( isset( $_POST[ 'add_service' ] ) ) {
	...
	if ( !$error ) {
		...
		//validate form
		//set variables
		$s_name = $_POST[ 'add_service_name' ];
		$s_description = $_POST[ 'add_service_description' ];
		$s_overview = $_POST[ 'add_service_overview' ];
		...
		if ( $page_body = mysqli_query( $dbc, $query ) ) {
			while ( $record = mysqli_fetch_array( $page_body ) ) {
				...
				$header = '<?php include("../common/header.php")?>';
				$database = '<?php include("../../DB/database.php"); ?>';
				$data = $record[ 'template_body' ];
				$social = '<?php include("../common/social.php")?>';
				$footer = '<?php include("../common/footer.php")?>';
		...
		//redirect user
		ob_end_clean();
		header( 'location: service-created.php' );
		exit();
